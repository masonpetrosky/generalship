...
def main():
    ...
    for s in added:
        ...
        for sec in selected:
            expected = '1867-12-24' if s['id'] == 'reid-preface-provenance-v1' else None
    ...
    for a in record['assertions']:
        ...
        for c in a['citations']:
            ...
            if c['inspection_method'] == 'facsimile':
                ...
                assert c['inspection_method'] == 'web_text_extraction' and s['format'] == 'json'
                snapshot = json.loads((ROOT / s['path']).read_text())
                assert any(c['quote'] in line for line in snapshot['selected_response_lines'])
                assert s['parent_sha256'] is None and c['document_date'] is None
    ...
    actual = dict(assertions=len(record['assertions']), passage_anchors=len(anchors),
                  ...
                  new_text_sections=len(sections), new_catalog_snapshots=sum(s['format'] == 'json' for s in added),
    ...
    assert actual == cov
    assert (cov['assertions'], cov['passage_anchors'], cov['source_locator_pairs'], cov['visually_inspected_pages']) == (5, 11, 10, 10)
    assert (cov['prior_source_entries_preserved'], cov['registry_entries'], cov['raw_paths']) == (152, 164, 161)
    assert all(value is None for key, value in record['conclusion'].items() if key != 'members_by_side')
