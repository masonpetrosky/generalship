...
def main():
    ...
    assert len(manifest['preserved_source_metadata_sha256']) == 111
    bound = set(manifest['preserved_source_metadata_sha256']) | set(substitutions.values())
    assert len(bound) == manifest['registry_entries_after'] == 112
    assert len({sources[s]['path'] for s in bound}) == manifest['raw_paths_after'] == 109
    assert count == 24 and changed_anchors == len(overlays) == 1
    ...
    print(json.dumps({'status': 'passed', 'preserved_source_entries': 111,
                      'new_corrected_versions': 1, 'exact_replacements': 2,
                      'replayed_anchors': count, 'changed_quote_anchors': changed_anchors,
                      'registry_entries_at_correction': 112, 'raw_paths_at_correction': 109,
