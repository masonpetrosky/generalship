# Atlanta Campaign first pass: separate review

Review prepared commit `1191d32433575d224120a6d2aa1df8e381dcecf7` against `4e705de56c131379d288356a7787f6e82b3e75c7`. Sibling inputs.json binds
82 paths at the prepared commit. You are a separate Claude Opus 5.5 `high` reviewer with
fresh context. The primary's conversation is not an input. Verify input hashes against
`git show 1191d32433575d224120a6d2aa1df8e381dcecf7:<path>` first.

Read AGENTS.md, `docs/methodology.md` (research depth and coverage), `docs/evidence-contract.md`,
`docs/cohort-v2.md`, the memo(s) `docs/research/atlanta-first-pass-v1.md`, the 16 new dossiers (GA007, GA008, GA009, GA010, GA011, GA012, GA013, GA014, GA015, GA016, GA017, GA018, GA019, GA020, GA021, GA022 in
`data/evidence/`) and the new source records in `data/sources.json` (the 47 added at the prepared
commit), with their selections under `data/raw/atlanta-v1/`.
...
4. **Sources.** Are the registry records accurate (edition, dates by section, independence groups
   and dependency notes, inspection notes that say what was read)? Are these choices sound: a new per-campaign independence group for authors registered earlier (Johnston, Cleburne, Hardee); GA013 (Marietta Operations) citing only context totals because Kolb's Farm and Kennesaw are separate frozen records inside it; the live NPS campaign label 'Morgan's Raid Into Kentucky' not adopted; the 1882 Cox edition?

Run `make check` offline. No build, packet or network commands; do not modify primary artifacts.

Write only `artifacts/review-results/atlanta-review-1191d32-opus-high-v1/review.md`; do not edit other files, commit or push. State the scope you actually
